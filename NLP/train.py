...
from NLP.batching import read_data, make_batches


TRAIN_DATA = './dataset/ptb/simple-examples/data/ptb.train'             # 训练数据路径
EVAL_DATA = './dataset/ptb/simple-examples/data/ptb.valid'             # 验证数据路径
TEST_DATA = './dataset/ptb/simple-examples/data/ptb.test'             # 测试数据路径
...
class PTBModel(object):
    def __init__(self, is_training, batch_size, num_steps):
        ...
        # 定义使用LSTM结构为循环体结构且使用dropout的深度循环神经网络。
        ...
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
            label=tf.reshape(self.targets, [-1]), logits=logits)
...
def main():
    ...
    with tf.variable_scope('language', reuse=True, initializer=initializer):
    ...
    with tf.Session() as sess:
        ...
        testing_batches = make_batches(read_data(TEST_DATA), EVAL_BATCH_SIZE, EVAL_NUM_STEP)
        ...
        _, test_pplx = run_epoch(sess, eval_model, testing_batches, tf.no_op(), False, 0)
