...
SHAPE_EMB_AND_SOFTMAX = True                # 在Softmax层和词向量层之间共享参数

# 词汇表中<sos>和<eos>的ID。
# 在解码过程中需要用<sos>作为第一步的输入，
# 并将检查是否是<eos>，因此需要知道这两个符号的ID
SOS_ID = 1
EOS_ID = 2
...
class NMTModel(object):
    ...
    def inference(self, src_input):
        # 虽然输入只有一个句子，
        # 但因为dynamic_rnn要求输入是batch的形式，
        # 因此这里将输入句子整理为大小为1的batch。
        ...
        with tf.variable_scope("decoder/rnn/multi_rnn_cell"):
            ...
            # 构建初始的循环状态。
            # 循环状态包含循环神经网络的隐藏状态，
            # 保存生成句子的TensorArray，以及记录解码步数的一个整数step
